...
public class UnaryNegativeOperator extends UnaryOperator {

    public UnaryNegativeOperator(String name, int precedence) {
    ...
    public Object render(RenderContext ctx, JtwigPosition pos, Object input) throws CalculateException {
        if (isDecimal(input)) {
            return toDecimal(input).negate();
        }
        return toLong(input) * -1L;
    }
